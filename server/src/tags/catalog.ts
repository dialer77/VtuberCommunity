...
//
// 태그 부여 = 자동 부트스트랩(스트리머 자가 태그에서 추출, 안전) + 관리자 수동 큐레이션.
// ⚠️ 퍼리·오토코노코 같은 캐릭터 속성은 자가 선언 또는 큐레이션된 경우에만 붙인다
//    (실존 인물에 임의 라벨링 금지).
// 참고: 공개 유저 제보는 도입하지 않는다(분탕·오분류 리스크) — 큐레이션은 관리자 전용.
...
// 관리자 수동 큐레이션 태그. channelId → tagId[]
// (자가 태그로 안 잡히는 컨셉 속성 등을 여기서 보강. 공개 제보는 받지 않음)
